...
# =========================================================
with tabs[3]:
    st.title("📘 Гайд по использованию")

    st.markdown("""
### Что делает программа
Приложение анализирует **TRC20 Transfers** (раздел *Transfers* на Tronscan), а не *Transactions*.

---

## 📊 Вкладка «Отчёт по кошельку»
**Когда использовать:** нужно быстро понять вход/выход по одному кошельку за период.

**Как пользоваться:**
1. Вставь TRON-кошелёк.
2. Выбери период (начало/конец).
3. Нажми **«Получить отчет»**.
4. Посмотри метрики и таблицу **Top-10 отправителей**.
5. Нажми **«Скачать Excel отчет»**.

**Метрики:**
- *Сумма входящих* — сумма всех переводов **на кошелёк**.
- *Кол-во входящих* — количество входящих transfers.
- *% топ-5* — доля суммы переводов от 5 крупнейших отправителей.
- *Сумма исходящих* — сумма переводов **с кошелька**.
- *Кол-во исходящих* — количество исходящих transfers.

---

## 🔄 Вкладка «Сравнение периодов (OLD → NEW)»
**Когда использовать:** кошелёк сменился (OLD → NEW), но клиенты те же.

**Как пользоваться:**
1. Введи **OLD wallet** и период для него.
2. Введи **NEW wallet** и период для него.
3. Нажми **«Сравнить периоды»**.

**Результат:**
- **Новые** — отправители есть в NEW, но не было в OLD.
- **Постоянные** — есть и в OLD, и в NEW.
- **Ушедшие** — были в OLD, но нет в NEW.

Также считаются суммы по группам и доступен Excel-отчёт.

---

## 🧩 Вкладка «Пересечение клиентов»
**Когда использовать:** нужно найти общих отправителей между двумя кошельками.

**Как пользоваться:**
1. Введи кошелёк A и период A.
2. Введи кошелёк B и период B.
3. Нажми **«Проверить пересечение»**.

**Результат:**
- Кол-во пересечений
- Сумма переводов от пересекающихся отправителей на A и на B
- Таблица:
  - From
  - SumA / SumB / Total
  - ExampleHashA / ExampleHashB (пример хеша перевода)

---

## 🔑 Tronscan API Key (опционально)
Если период большой или много переводов, Tronscan может ограничивать запросы.
В таком случае вставь свой API key в поле **Tronscan API Key (optional)**.

---

## 📥 Excel
Каждая вкладка умеет выгружать Excel:
- **Отчёт по кошельку** → Summary + Top10
- **Сравнение** → Summary + New + Persistent + Lost
- **Пересечения** → Summary + Intersection
""")

    st.info("Совет: если период слишком большой и данных много — разбивай анализ на части (например, по месяцам).")
